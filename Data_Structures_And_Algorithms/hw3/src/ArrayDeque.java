...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        if (size  == backingArray.length) {
            //copy elements to new array of double capacity
            T[] newArray = (T[]) new Object[backingArray.length * 2];
            for (int i = 0; i < size; i++) {
                newArray[i + 1] = backingArray[mod(front + i, backingArray.length)];
            }
            front = 0;
            newArray[mod(front, backingArray.length)] = data;
    ...
    public T removeFirst() {
        ...
        T removedElem = backingArray[mod(front, backingArray.length)];
    ...
    public T removeLast() {
        ...
        T removedElem = backingArray[mod(size + front - 1, backingArray.length)];
    ...
    public T getFirst() {
        //exceptions
        if (size == 0) {
            throw new NoSuchElementException("Cannot retrieve element from empty list");
        }
    ...
    public T getLast() {
        ...
        return backingArray[mod(size + front - 1, backingArray.length)];
    }
