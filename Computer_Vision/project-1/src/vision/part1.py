...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    for i in range(-mid, mid+1):
      curr = (float)(1/(np.sqrt(2 * np.pi) * sigma)) * np.exp(-((i**2)/(2 * (sigma ** 2)))) #remove (float) for jupyter
      kernel[index] = curr
      sum = sum + curr
      index += 1
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    kernel = np.matmul(create_Gaussian_kernel_1D(cutoff_frequency * 4 + 1, cutoff_frequency), np.transpose(create_Gaussian_kernel_1D(cutoff_frequency * 4 + 1, cutoff_frequency)))
    ### END OF STUDENT CODE ####
    ############################
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    filterR, filterC = filter.shape
    padM = (int)((filterR - 1) / 2)
    padN = (int)((filterC - 1) / 2)
    imgR, imgC, imgD = image.shape
    newImg = np.zeros((imgR + padM * 2, imgC + padN * 2, imgD))
    filtered_image = np.zeros((imgR, imgC, imgD))
    for i in range (padM * 2 + imgR):
      for j in range (padN * 2 + imgC):
        if (i < padM or i > padM + imgR - 1 or j < padN or j > padN + imgC - 1):
          continue
        else:
          newImg[i][j] = image[i - padM][j - padN]
    for i in range(imgR):
      for j in range(imgC):
        filtered_image[i][j][0] = np.multiply(newImg[i:i + padM * 2 + 1, j:j + padN * 2 + 1, 0], filter).sum()
        filtered_image[i][j][1] = np.multiply(newImg[i:i + padM * 2 + 1, j:j + padN * 2 + 1, 1], filter).sum()
        filtered_image[i][j][2] = np.multiply(newImg[i:i + padM * 2 + 1, j:j + padN * 2 + 1, 2], filter).sum()
    #print(newImg[0:0 + padM * 2 + 1, 0:0 + padN * 2 + 1, 0])
    #print(filter)
    #print(np.dot(newImg[i:i + padM * 2 + 1, j:j + padN * 2 + 1, 0], filter))
    ### END OF STUDENT CODE ####
    ############################
...
def create_hybrid_image(
    ...
    hybrid_image = np.clip(np.add(low_frequencies, high_frequencies), 0, 1)
    ### END OF STUDENT CODE ####
    ############################
