...
def my_conv2d_pytorch(image: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
    ...
    padR = (int)((kernel.size()[2] - 1) / 2)
    padC = (int)((kernel.size()[3] - 1) / 2)
    filtered_image = torch.nn.functional.conv2d(input=image, weight=(kernel), padding=((int)(padR), (int)(padC)), groups=(int)(image.size()[1]))
    ### END OF STUDENT CODE ####
    ############################
