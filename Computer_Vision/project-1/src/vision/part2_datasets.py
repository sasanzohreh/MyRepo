...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    for file in listdir:
        ...
        if (file.__contains__("a_")):
            images_a.append(image)
        elif(file.__contains__("b_")):
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    for line in lines:
        cutoff_frequencies.append((int)(line.replace("\n", "")))
    file.close()
...
class HybridImageDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        fileA = self.images_a[idx]
        fileB = self.images_b[idx]
        fileA = Image.open(fileA)
        fileB = Image.open(fileB)
        image_a = self.transform(fileA)
        image_b = self.transform(fileB)
