...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernelR, kernelC = kernel.shape
        c = self.n_channels
        kern = np.zeros((c, 1, kernelR, kernelC))
        for i in range(c):
          kern[i][0] = kernel
        kernel = kern
        kernel = torch.tensor(kernel)
        kernel = kernel.float() #remove for jupyter
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        ### TODO: YOUR CODE HERE ###
        kernel = kernel.float()
        filtered_image = torch.nn.functional.conv2d(x, kernel, padding='same', groups=self.n_channels)
