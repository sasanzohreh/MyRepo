...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            beliefs[position] = self.getObservationProb(observation, gameState.getPacmanPosition(),
                    position, self.getJailPosition()) * self.beliefs[position]
    ...
    def elapseTime(self, gameState):
        ...
        for position in self.allPositions:
            ...
            for newPos, prob in newPosDist.items():
                beliefs[newPos] += (self.beliefs[position] * prob)
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.legalPositions:
            beliefs[position] = self.getObservationProb(observation, gameState.getPacmanPosition(),
                position, self.getJailPosition()) * self.getBeliefDistribution()[position]
        beliefs.normalize()
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            ...
            for i in range(self.numParticles):
                resample.append(beliefs.sample())
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            resample.append(newPosDist.sample())
        self.particles = resample
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            belief = 1
            for i in range(self.numGhosts):
                belief *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
