    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        closestGhost = [None, None]
        for dist in livingGhostPositionDistributions:
            max = [None, None]
            for key, value in dist.items():
                if max[1] is None or value >= max[1]:
                    max[0] = key
                    max[1] = value
            distance = self.distancer.getDistance(pacmanPosition, max[0])
            if closestGhost[1] is None or (distance <= closestGhost[1]):
                closestGhost[1] = distance
                closestGhost[0] = max[0]
        min = [None, None]
        for action in legal:
            distance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closestGhost[0])
            if min[1] is None or distance <= min[1]:
                min[0] = action
                min[1] = distance
        return min[0]
