"I'd like to use 1 late day for Project 4"
import copy
import sys
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            nextActs = []
            for node in layer:
                value = node.sigmoidActivation(current)
                nextActs.append(value)
            output.append(nextActs)
            current = nextActs
        return output
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in range(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) #entry
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) # entry
                    weightedSum = 0
                    for i, nextDelta in enumerate(deltas[0]):
                        #weightedSum += self.layers[layerNum][i].weights[layerNum + 1] * deltas[layerNum][i]
                        weightedSum += nextDelta * nextLayer[i].weights[neuronNum+1]
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while abs(weightMod) >= weightChangeThreshold and iteration <= maxItr:
